...
export const columns = ({
  ...
  visibleFeatures: DB_FeaturesType[];
}): ColumnDef<DB_FeaturesType>[] => [
  // Name
  {
    ...columnId({ id: "name" }),
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="p-2">
          <Link
            href={`${featuresTypeMeta.href}/${slug}`}
            className={
  ...
  // Slug
  {
    ...
    cell: ({ row }) => (
      <div className="p-2">
        <Link
          href={`${featuresTypeMeta.href}/${row.original.slug}`}
  ...
  // Created At
  {
    ...
    cell: ({ row }) => {
      ...
      return <div className="p-2">{dateFormatter({ date })}</div>;
    },
  },
  // Created By
  {
  ...
  // Actions
  {
    ...
    cell: ({ row }) => {
      const lpFeature = row.original;

      return (
        <div className="flex items-center justify-start p-2">
          <LandingPageFeaturesRowActions lpFeature={lpFeature} />
        </div>
      );
    },
  },
