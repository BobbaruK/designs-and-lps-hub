...
export const brandDownloadsWhere = ({
...
// TODO: URGENT!!!! refactor this shit for every resource across the app

function buildPrismaFilter(
  operator: "AND" | "OR" = "AND",
  resourcesArr: ResourceToFilter[],
) {
  const filters: Prisma.dl_brand_resourceWhereInput[] = [];

  const filterMeta: Record<string, { some: boolean; subKey: SubKey }> = {
    type: { some: false, subKey: "in" },
  ...
  const addFilter = (opts: {
    ...
    } else if (Array.isArray(opts.values) && opts.values.length > 0) {
      filters.push({
        [opts.key]: {
          ...(opts.some
            ? {
