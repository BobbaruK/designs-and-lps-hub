...
}): Prisma.dl_languageWhereInput => {
  const resourcesToFilter: ResourceToFilter[] = [];

  if (search !== null) {
    resourcesToFilter.push({ englishName: search });
  }
...
function buildPrismaFilter(
  ...
  const filters: Prisma.dl_languageWhereInput[] = [];

  const filterMeta: Record<string, { some: boolean; subKey: SubKey }> = {
    englishName: { some: false, subKey: "contains" },
